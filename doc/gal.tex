...
Množina vrcholů $S \subseteq V$ neorientovaného grafu $G(V,E)$ je nezávilá, když pro každé dva vrcholy $\forall M,N \in S$ platí, že nejsou spojeny hranou $(M,N) \notin E$ . Dále definujeme množinu $S$ jako maximální právě když $Adj(S) \cup S = V$, kde $Adj(S)$ značí množinu sousedních uzlů k uzlům z $S$.
...
\While{True}
  ...
  \If {$N_k \neq \emptyset \lor S_K \neq \emptyset$}
    ...
    \If{$N_k \neq \emptyset$}
      ...
      \If{$ \forall y \in S_k : \ Adj(y) \cap S_k \neq \emptyset $}
